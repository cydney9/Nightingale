...
public:
	...
	//*Can be enviroment, enemy, player, friendly
	unsigned int GetBodyID() const;
	//What types does this body collide with?
	//*Can be enviromentm, enemy, player, and friendly
	...
	//Set position (just sets the variable, doesn't actually set position)
	void SetPosition(b2Vec2 bodyPos);
	//set IsGrounded
	void SetIsGrounded(bool Gro);
private:
	...
	//Is gravity beging applied to this body?
	bool m_applyGravity = false;

	//Is the objecy grounded?
